...
{
    public class CZBombBot : BotBase
    {
        ...
        public CZBombBot(TokenConfig config, DiscordSocketClient client, CZBombOracle moneyOracle, CZBombTreasury moneyTreasury,
            IReadOnlyCollection<SocketGuild> socketGuilds)
            : base(config, client, moneyOracle, moneyTreasury, socketGuilds)
        {
            Logging.WriteToConsole("Loading CZBombBot...");
        }
        ...
        private async Task AsyncGetTWAP()
        {
            ...
            while (true)
            {
                try
                {
                    var twapD = await ((CZBombOracle)Oracle).TWAPAsync();
        ...
        private async Task<object> AsyncGetLastEpochTWAP()
        {
            ...
            while (true)
            {
                try
                {
                    var consultD = await ((CZBombTreasury)Treasury).PreviousEpochBombPriceAsync();
